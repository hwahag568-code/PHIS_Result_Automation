...
최신 Release는 [Latest Release](https://github.com/hwahag568-code/PHIS_Result_Automation/releases/latest)에서
확인할 수 있습니다. 설치된 프로그램은 실행 시 새 Release를 자동으로 확인하며,
PHIS 작업 중에 확인된 업데이트는 작업이 끝난 뒤 처리합니다. GUI에는 수동
업데이트 확인 버튼이나 마지막 확인 시각을 표시하지 않습니다.

신규 설치와 수동 복구에는 전체 설치파일이 포함된 Release의
...
사용합니다. 업데이트 전용 Release에 호환 패치가 없거나 런타임 세대가
다르면 해당 Release를 자동 적용하지 않습니다. 경량 업데이트 지원 이전 버전에서는 먼저 전체 설치파일로
지원 버전을 한 번 설치해야 하며, 이후 같은 런타임 세대의 업데이트부터
경량 패치를 사용할 수 있습니다.

각 Release의 태그와 설치파일 이름은 버전마다 고유합니다. 이미 공개한
Release 파일을 같은 태그로 교체하지 않으며, 수정본은 새 버전으로
배포합니다. 일반 코드 수정 Release는 경량 업데이트만 제공할 수 있으며,
런타임 또는 설치 방식이 바뀌는 Release에는 전체 설치파일을 제공합니다.
각 Release에는 런타임 세대와 자산 구성을 기록한 manifest도 함께 제공합니다.
